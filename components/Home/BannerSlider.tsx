...
export default function BannerSlider() {
  ...
  // ---------- Performance: pause slider when tab hidden ----------
  useEffect(() => {
  ...
  // ---------- Navigation handlers (same logic) ----------
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-neutral-950">
      {/* ---------- SLIDE CONTAINER with CSS transitions ---------- */}
      <div
        key={currentIndex}
        className="absolute inset-0 w-full h-full opacity-0"
        style={{
          opacity: 1, // overridden by inline style? We'll control opacity via state later
          transition: "opacity 1.2s cubic-bezier(0.33, 1, 0.68, 1)",
          willChange: "opacity",
        }}
        // Using a ref to force re-triggering animation; simpler: just render the slide
        // We'll add a class that triggers the zoom-out animation
      >
        {/* Background image with CSS zoom animation */}
        <div
          className="absolute inset-0 bg-cover bg-center animate-zoom-out"
          style={{
            backgroundImage: `url('${slides[currentIndex].image}')`,
            animation: "zoomOut 6s cubic-bezier(0.33, 1, 0.68, 1) forwards",
          }}
        />
        ...
        <div className="relative h-full flex items-center justify-center z-10">
          <div className="text-center px-6 max-w-5xl mx-auto flex flex-col items-center select-none">
            {/* Animated Accent Header – CSS animation */}
            ...
            {/* Animated Main Subtitle – CSS animation with blur effect */}
            ...
            <div className="opacity-0 animate-fade-in-up-3">
              <Link href={slides[currentIndex].path}>
                <button className="inline-flex items-center justify-center px-10 py-4 bg-gradient-to-b from-white/10 to-white/[0.02] hover:from-white hover:to-white border border-white/20 hover:border-white text-white hover:text-black font-serif font-medium text-xs sm:text-sm uppercase tracking-[0.3em] transition-all duration-500 rounded-none transform hover:scale-102 cursor-pointer shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                  {/* BACKDROP-BLUR REMOVED – replaced by solid glass-like background */}
      ...
      {/* ---------- Navigation Arrows (blur removed) ---------- */}
      <button
        onClick={goToPrev}
      ...
      {/* Injected CSS keyframes (scoped to this component via a <style> tag or add to global styles) */}
      <style jsx>{`
        @keyframes zoomOut {
        ...
        /* Fade-in + move up with delays */
        .animate-fade-in-up-1 {
          animation: fadeInUp 0.8s ease 0.2s forwards;
        }
        .animate-fade-in-blur-2 {
          animation: fadeInBlur 1s cubic-bezier(0.16, 1, 0.3, 1) 0.3s forwards;
        }
