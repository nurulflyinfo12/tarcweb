...
export default function Page() {
  ...
  // Safely extract slug (Next.js 15+ compatibility)
  useEffect(() => {
    if (params?.slug) {
      setRestaurantId(params.slug as string);
    }
  }, [params]);
          ...
          <div className="relative bg-white rounded-2xl grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center p-5 sm:p-6 md:p-8 lg:p-10 overflow-hidden">
            ...
            >
              <div className="flex items-center justify-center lg:justify-start gap-2 sm:gap-3 flex-wrap">
                ...
                <h2 className="text-2xl sm:text-3xl md:text-3xl lg:text-3xl font-medium text-secondary tracking-wide">
                  {restaurant.name}
                </h2>
                <span className="h-[1px] w-8 sm:w-12 bg-[#556B2F]" />
              </div>
